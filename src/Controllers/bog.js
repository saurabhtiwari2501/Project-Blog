...
const updateBlog = async function (req, res) {
    ...
    if(blog.isDeleted == true){
      return res.status(404).send({status : false , msh: "may be blog is deleted"})
    }

    if (blog.isPublished == true){
      return res.status(404).send({stastus : false , msg : "may be is already published"})
    }

    let newBlog = await blogModel.findOneAndUpdate(
      blogID,
      { $set: {
        title,
        body,
